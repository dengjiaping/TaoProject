...
import com.crazy.taolove.activity.VoipCallActivity;
...
import com.crazy.taolove.entity.PushMsgModel;
	...
	public synchronized void handlePushMsg(boolean isPassThroughMsg, String pushMsgJson) {
		...
		if (pushMsgModel != null && !TextUtils.isEmpty(pushMsgModel.sender)) {
			if (pushMsgModel.msgType == PushMsgModel.MessageType.VOIP) {
				if (!AppManager.getTopActivity(CSApplication.getInstance()).equals("com.crazy.taolove.activity.VoipCallActivity")) {
